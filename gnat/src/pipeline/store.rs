...
impl StoreProcessor {
    pub fn new(
        ...
        options.entry("hive").or_insert("false");
        ...
        let mut hive_type = options.get("hive").expect("expected hive type").to_string();
         if !hive_type.is_empty() {
            if hive_type == "false" {
               is_hive = false;              
            } else if hive_type == "true" {
                 is_hive = true;
            }
            else {
                return Err(Error::other("hive option must be true or false"));
            }
        }
            ...
            if self.filter.is_empty() {
                ...
                if self.is_hive {
                    sql_s3_copy = format!(
                        "COPY (SELECT *, year(stime) AS year, month(stime) AS month, day(stime) as day FROM flow
                         WHERE year = {} AND month = {} AND day = {}) 
                         TO '{}/gnat{}{}{}-{{uuid}}.parquet' (FORMAT 'parquet');",  
                        dtg.year, dtg.month, dtg.day,
                        self.output_list[0],
                        dtg.year, dtg.month, dtg.day);
                }
