                             ...
                             uint16_t risk_threshold)
{
    ...
    duckdb_timestamp start = {(flow->flowStartMilliseconds * 1000)}; // microseconds
    duckdb_append_timestamp(appender, start);
    duckdb_timestamp end = {(flow->flowEndMilliseconds * 1000)}; // microseconds
    ...
    g_string_truncate(buffer, 0);
    {
        // Calculate total non-empty packets (capped at 8)
        uint32_t total_non_empty = flow->nonEmptyPacketCount + flow->reverseNonEmptyPacketCount;
        if (total_non_empty > 8)
            total_non_empty = 8;

        // Pad with '.' on left, populate 0 or 1 right to left for packets with payload
        for (int i = 0; i < 8; i++)
        {
            if ((uint32_t)i < (8 - total_non_empty))
            {
                g_string_append_c(buffer, '.');
            }
            else
            {
                g_string_append_c(buffer, (flow->firstEightNonEmptyPacketDirections & (1 << (7 - i))) ? '1' : '0');
            }
        }
    }
    duckdb_append_varchar(appender, buffer->str);
